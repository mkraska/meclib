...
<li><code>[ "dir", "&lt;name&gt;", [x,y], angle, offset, length]</code> small arrow with label (indication of coordinate axes). Offset (defaults to 10 pix) and length (defaults to 1) are optional. If offset is negative, the label is placed at the tail instead of the head.</li>
