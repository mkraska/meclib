...
class rope {
  constructor(data) {
    ...
    let vis = {visible:false}; // use spread operator ( ...vis ) for elements with multiple attributes, otherwise just use variable itself
	  this.p1=board.create("point", c1, vis );
    this.ci1 = board.create("circle",[this.p1,r1], vis)
    this.p2=board.create("point", c2, vis );
    this.ci2 =board.create("circle",[this.p2,r2], vis);
    let num=1;
    // equal radius
    if (r1==r2) {
      this.t1= board.create("segment",[this.p1,this.p2],vis)
      this.n1= board.create("perpendicular",[this.t1,this.p1],vis)
      this.i2=board.create("intersection",[this.n1,this.ci1],vis)
      if (r1<0) {this.i2=board.create("otherintersection",[this.n1,this.ci1,this.i2],vis)}
      var points = [this.p1, this.p2, this.i2]
    }
    // works for abs(r1)>=abs(r2) and all combinations of signs except for r1==r2
    else if (Math.abs(r1)>=Math.abs(r2)) {
      let pm = board.create("midpoint", [this.p1,this.p2],{name:'M',...vis})
      let cm =board.create("circle", [pm,this.p1], vis)
      let cdif=board.create("circle",[this.p1,r1-r2], vis)
      let i1=board.create("intersection",[cm,cdif], vis)
      if (r1>0) {i1=board.create("otherintersection",[cm,cdif,i1], vis)}
      let n1=board.create("normal",[cdif,i1],vis)
      let i2=board.create("intersection",[n1,this.ci1],vis) 
      var points = [i1, this.p2, i2]
    }
    // works for abs(r1)<abs(r2) and all combinations of signs
    else if (Math.abs(r1)<Math.abs(r2)) {
      if (r2>0) { num=0} 
      let pm = board.create("midpoint", [this.p1,this.p2], vis)
      let cm =board.create("circle", [pm,this.p1],vis)
      let cdif=board.create("circle",[this.p2,r2-r1],vis)
      let i1=board.create("intersection",[cm,cdif],vis)
      if (r2<0) {i1=board.create("otherintersection",[cm,cdif,i1],vis)}
      let n1=board.create("normal",[cdif,i1],vis)
      let i2=board.create("intersection",[n1,this.ci2],vis) 
      var points = [i1, this.p1, i2]
    }
    // line with label
    this.l = board.create('parallel', points, {
      straightFirst: false, straightLast: false,
      layer: defaultMecLayer, ...normalStyle
    })
    this.label = board.create("midpoint",[this.l],
      {name:data[1],label:{autoposition:true, autoPositionMinDistance:6,anchorX: 'middle',
        anchorY: 'middle'},showinfobox:false,size:0}) 
    // target elements for load application
    targets.push(this.l);
    // implement state switching
    this.obj = [this.l, this.label.label];  
    // state init
    switch (this.state) {
  ...
  data(){ return [ "rope", this.d[1], XY(this.p1), this.d[3], XY(this.p2),this.d[5], this.state]}
  name(){ return targetName(this) } 
  hasPoint(pt) {
    return isOn(pt,this.l) && 
    JXG.Math.Geometry.distPointLine( [1,pt.X(),pt.Y()], this.l.stdform) < tolPointLine} 
  }
